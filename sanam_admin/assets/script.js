  ...
  // إنشاء HTML لجدول مع thead/thead لتكرار العناوين في الطباعة
  ...
  async function loadAndRender(){
    ...
    const sheet2Raw=await fetchSheetData('1zq07aJ_9w27WW0av_Qxrq6ZBWb1uuHHWeuRy-lS9aI0','البيانات!A2:H');
    ...
    // معاينة على الشاشة
    document.getElementById('securityCasesPreview').innerHTML=buildTableHTML(filtered1);
    document.getElementById('attendanceRecordsPreview').innerHTML=buildTableHTML(filtered2);
  ...
  // تقدير تقريبي (لم نعد نستخدمه للحقل ولكنه مفيد لو أردت إظهار تقدير قبل البناء)
  ...
  function buildPrintPages(values1, values2){
    ...
    // نص الغلاف (يقبل سطرين أو أكثر بفضل white-space:pre-wrap في CSS)
    ...
    // (3) صفحات الجداول (قياس ديناميكي حتى لا يُكسر الصف)
    appendPaginatedTableMeasured(root, values1, 'ملخص الحالات الأمنية', 20);
    appendPaginatedTableMeasured(root, values2, 'ملخص الحضور والانصراف', 20);
  }

  // بناء صفحات الجداول مع قياس الصفوف التي تلائم الصفحة فعلاً
  function appendPaginatedTableMeasured(root, values, title, extraBottomMm=20){
    ...
    pages.forEach((vals, idx) => {
      ...
      const tableHTML = buildTableHTML(vals);
  ...
  function syncPrintFields(){
    ...
    // مساعد لتحويل الأسطر إلى فقرات <p> مع اتجاه RTL وخيارات محاذاة/سمك
    ...
    // (ج) التذييل: RTL مع محاذاة لليسار وبخط عريض
  ...
  function finalizePrintPages(){
    ...
    // (1) تأكيد عدم كسر بعد الصفحة الأخيرة (منع صفحة فارغة أخيرة)
    if (pages.length) {
    ...
    // (2) حذف أي صفحة لا تحتوي محتوى فعلي (احتياط نادر)
    pages.forEach(p => {
      const hasContent =
